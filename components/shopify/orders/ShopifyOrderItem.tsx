...
const ShopifyOrderItem: React.FC<ShopifyOrderItemProps> = ({ order, handleClick, className }) => {
  return (
    <Button
      variant="ghost"
      className={cn("w-full justify-start text-left hover:bg-accent", className)}
      ...
      <div className="flex items-center space-x-4">
        <div className="flex-shrink-0">
          <Image
            ...
            src={order?.lineItems?.edges[0]?.node?.variant?.image?.url || '/placeholder.svg'}
            width={64}
            height={64}
