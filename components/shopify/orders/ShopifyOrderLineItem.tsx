...
const ShopifyOrderLineItem: React.FC<ShopifyOrderLineItemProps> = ({ lineItem, className }) => {
  return (
    <div className={cn("flex items-start space-x-4 py-2", className)}>
      ...
      <div className="flex-grow">
        ...
        <p className="text-sm text-muted-foreground">{lineItem?.variant?.title}</p>
        <p className="text-sm text-muted-foreground">
          {lineItem.quantity} x {formatCurrency(lineItem?.variant?.price?.amount)}
        </p>
      </div>
    </div>
  )
