  ...
  const renderElement = (el, key, inList) => {
    switch (el.type) {
      ...
      case 'heading':
        ...
        return (
          <HeadingTag key={key} className={cn(
            "font-bold",
            el.level === 1 && "text-4xl",
            el.level === 2 && "text-3xl",
            el.level === 3 && "text-2xl",
      ...
      case 'list-item':
        return <li key={key} className="mb-1">{renderValue(el.children, true)}</li>
      case 'link':
        return (
          <Link key={key} href={el.url} title={el.title} target={el.target} className="text-blue-600 hover:underline">
            {renderValue(el.children, inList)}
          </Link>
        )
      case 'text':
